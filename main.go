...
func postItemsByUserHandle(handle string) ([]FeedItemPost, error) {
    ...
    if err != nil { return nil, fmt.Errorf("postItemsByUserHandle %v", err) }
    defer rows.Close()
...
func latestPosts() ([]FeedItem, error) {
        ...
        for rows.Next() {
            ...
            if author_display_picture.Valid {feedItem.User.Display_picture = author_display_picture.String}
